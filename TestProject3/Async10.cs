...
namespace csharp_tutorial2
{
    public class Async10
    {
        ...
        public async Task Tasks_FromAsync_Linq()
        {
            ...
            // Try what happens without .ToList() and with it
            var over10Sensors = tasksLinq.Where(e => e.Result.Data > 10).ToList();
        }
        
        // Sometimes is really hard ti understand how Framework behaves
        // Examples have blocking Sleep before and after await Delay
        [Fact]
        public async Task Threading_Is_Hard()
        {
            var particallyBlocking = LongRunningWithSync_Blocking();

            await particallyBlocking;
