...
namespace csharp_tutorial2
{
    public class Parallel13
    {
        ...
        public async Task AsyncAwaitParallel()
        {
            ...
            // This is not much faster, because AsParallel already runs these in parallel
            ...
            // This wont be parallel as this is wrong: foreach (int i in idList.AsParallel())
        ...
        public async Task AsyncAwaitForEachSelect()
        {
            ...
            var slowResults = await Task.WhenAll(slowTasks);
            ...
            var asyncActions2 = idList.Select(async e => SlowAsyncAction1Sec(e)).ToList();

            await Task.WhenAll(asyncActions2);

            asyncActions2.ForEach(e => Trace.WriteLine("Result: " + e.Result));

            Trace.WriteLine($"Select sync: {sw.ElapsedMilliseconds}ms");
        }
        ...
        public void Tasks()
        {
            ...
            // Task.Run is a shorthand for Task.Factory.StartNew with some default arguments
