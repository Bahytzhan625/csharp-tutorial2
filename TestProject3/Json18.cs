...
using csharp_tutorial2.Helpers.Hsl;
using Xunit;
        ...
        public static async Task<dynamic> GetLocation(SearchType searchType, string reference)
        {
            using (var client = new HttpClient())
            {
                ...
                var locations = JObject.Parse(jsonData)
                    ...
                    .Where(s => s.SelectToken($"MonitoredVehicleJourney.{searchType.ToString()}.value").ToString() ==
                                reference)
                ...
                var locationFromModel = data.Siri.ServiceDelivery.VehicleMonitoringDelivery
